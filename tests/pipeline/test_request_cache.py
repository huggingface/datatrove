...
def test_request_cache_store_and_fetch(tmp_path):
    async def _run():
        ...
        assert await cache.get_cached_response("doc-1", 0, payload) == (None, None)

        await cache.store_result(0, "doc-1", 0, payload, result)
        await cache.store_error(0, "doc-2", 0, payload, "boom")

        resume_cache = RequestCache(str(tmp_path))
        await resume_cache.initialize(rank=0)

        cached_result, cached_error = await resume_cache.get_cached_response("doc-1", 0, payload)
        assert cached_error is None
        assert cached_result == result
        cached_result, cached_error = await resume_cache.get_cached_response("doc-2", 0, payload)
        assert cached_result is None
        assert cached_error == "boom"

        await resume_cache.mark_document_complete("doc-1")
        assert await resume_cache.get_cached_response("doc-1", 0, payload) == (None, None)
