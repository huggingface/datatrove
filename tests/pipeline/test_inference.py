...
def test_chunked_checkpoint_requires_chunk_index(tmp_path):
    ...
    with pytest.raises(ValueError, match="chunk_index"):
        InferenceRunner(
            ...
            config=config,
            output_writer=JsonlWriter(str(tmp_path / "no_chunk"), output_filename="${rank}.jsonl", compression=None),
...
def test_query_builder_none_payload_skips_document(tmp_path):
    ...
    output_file = output_dir / "00000.jsonl"
    assert not output_file.exists() or output_file.read_text().strip() == "", "No output should be written for skipped document"
...
def test_async_query_builder_none_payload_skips_document(tmp_path):
    ...
    doc = documents[0]
    assert not doc.metadata.get("inference_results"), "Document should have no inference results when async payload is None"
    output_file = output_dir / "00000.jsonl"
    assert not output_file.exists() or output_file.read_text().strip() == "", "No output should be written for skipped document"
