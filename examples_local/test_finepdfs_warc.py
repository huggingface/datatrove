#!/usr/bin/env python3
"""
Test FinePDFs Pipeline with Local WARC Files

First complete end-to-end test with WARC files:
- Stage 1: PDFWarcReader → PDFTruncationDetector → PDFRouter → Save
- Stage 2: Docling extraction for low OCR PDFs
- Stage 3: RolmOCR extraction for high OCR PDFs

Uses 3 local CommonCrawl WARC files from 2018.
"""
...
def test_finepdfs_warc():
    """Test full FinePDFs pipeline with local WARC files."""

    print("=" * 80)
    print("FinePDFs Pipeline Test - WARC Files")
    print("=" * 80)
    print(f"\nConfiguration:")
    print(f"  WARC Files: {WARC_DATA_FOLDER}/*.warc.gz")
