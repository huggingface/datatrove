...
class BaseMediaWriter(PipelineStep, ABC):
    ...
    @abstractmethod
    def _write(self, media: Media, file_handler: IO, filename: str) -> tuple[str, int]:
        """
    ...
    def write(self, media: Media, rank: int = 0, **kwargs):
        ...
        """
        original_name = output_filename = self._get_output_filename(media, rank, **kwargs)
        ...
        # actually write
        filename, offset = self._write(media, self.output_mg.get_file(output_filename), output_filename)
