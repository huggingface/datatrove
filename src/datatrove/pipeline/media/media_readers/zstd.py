...
class ZstdThreadedReader(BinaryReaderThreaded):
    ...
    def _read_without_length(self, zstd_decompressor_reader: zstd.ZstdDecompressor, media_id: str, offset: int, current_thread_name: str, fp: IO) -> bytes | None:
        """
    ...
    def _read_with_length(self, zstd_decompressor_reader: zstd.ZstdDecompressor, offset: int, length: int, fp: IO) -> bytes | None:
        """
        ...
        """
        fp.seek(offset)
        with zstd_decompressor_reader.stream_reader(LimitedReader(fp, length), read_across_frames=False, closefd=False) as zstd_stream_reader:
    ...
    def read_media_record(self, media: Media) -> bytes | None:
        ...
        if length is None:
            return self._read_without_length(self.thread_local.current_zstd_decompressor, media.id, offset, current_thread_name, fp)
        else:
