...
class FileTokenizerMerger(PipelineStep):
    """ Merge/shuffle a folder of tokenized files into a sequence of files with a maximum number of tokens per file.
        ...
        NOTE: This pipeline step only shuffle the order of the files which are merged together. It doesn't shuffle the order
        of the documents accross the files. If you want to shuffle the documents in a more extensive way or if you have
        a small number of files to merge after the DocumentTokenizer step, you should use the DocumentTokenizerMerger.
    ...
    def get_ordering(self, files: list[int]) -> Union[np.ndarray, List[int]]:
        """ Get the ordering of the files to process.
            If shuffle is True, the files are shuffled, otherwise they are returned as is.
        """
        return files if not self.shuffle else self.rand.permutation(files)
    ...
    def run(self, data: DocumentsPipeline = None, rank: int = 0, world_size: int = 1) -> DocumentsPipeline:
        ...
        file_ordering = self.get_ordering(range(len(datafiles)))
        ordered_doc_ids = []
        ordered_doc_ends = []
        current_max_tokens = 0
        for file_id in file_ordering:
            doc_ends = load_doc_ends(self.input_folder.open(datafiles_index[file_id], "rb"))
            if 0 < self.max_tokens_per_file <= doc_ends[-1] + current_max_tokens:
                # We have a file boundary in this file

            ordered_doc_ends.extend((doc_ends + current_max_tokens).tolist())
            ordered_doc_ids.extend((np.ones(len(doc_ends)).tolist() * file_id).tolist())
            current_max_tokens = doc_ends[-1]
...
def load_doc_ends(file: BinaryIO) -> list[int]:
