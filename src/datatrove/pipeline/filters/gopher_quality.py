...
class GopherQuality(BaseFilter):
    ...
    def __init__(
        ...
        @param stop_words:
        """
        super(GopherQuality, self).__init__(*args, **kwargs)
    ...
    def filter(self, doc: Document) -> FilterResult:
        ...
        if n_words < self.min_doc_words and self.min_doc_words:
            return FilterResult(False, "gopher_short_doc")
        if n_words > self.max_doc_words and self.max_doc_words:
            return FilterResult(False, "gopher_long_doc")
        ...
        if avg_n_words < self.min_avg_word_length and self.min_avg_word_length:
            return FilterResult(False, "gopher_below_avg_threshold")
        if avg_n_words > self.max_avg_word_length and self.max_avg_word_length:
            return FilterResult(False, "gopher_above_avg_threshold")
        ...
        if text.count("#") / n_words > self.max_symbol_word_ratio and self.max_symbol_word_ratio:
            return FilterResult(False, "gopher_too_many_hashes")
        if text.count("...") / n_words > self.max_symbol_word_ratio and self.max_symbol_word_ratio:
            return FilterResult(False, "gopher_too_many_ellipsis")
