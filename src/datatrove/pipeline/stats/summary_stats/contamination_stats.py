...
class WordsContaminationStats(SummaryStats):
    """
    ...
    def __init__(
        ...
        super().__init__(
            output_folder, groups_to_compute, histogram_round_digits, top_k_config=top_k_config
        )
        if len(words) == 0:
    ...
    def extract_stats(self, doc: Document) -> dict[str, int | float]:
        ...
        return {
            f"words_contamination_{self.words[0]}": sum(
            [1 for word in doc_words if word in self.words]
            ) / len(doc_words)
        }
