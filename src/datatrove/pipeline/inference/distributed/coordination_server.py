...
from datatrove.pipeline.inference.distributed.utils import get_master_node_host
...
class CoordinationServer:
    ...
    - /port: Get the inference server port on master node
    """
    ...
    async def master_running(self, timeout: float = 10.0) -> bool:
        ...
        try:
            status_code, _ = await asyncio.wait_for(_raw_get(f"http://{self.master_node_host}:{self.port}/health/{self.job_id}"), timeout=timeout)
        except Exception:
            return False
