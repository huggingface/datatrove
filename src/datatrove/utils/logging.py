...
def add_task_logger(
    ...
    Args:
      ...
      node_rank: int: Node rank for logging prefix. Logs will be prefixed with [NODE X], we assume node_rank == 0 is the master node.
    Returns:
    ...
    # Create format string with node prefix at the start
    # Format: [NODE X] timestamp | level | module:function:line - message
    node_prefix = f"[NODE {node_rank}] "
